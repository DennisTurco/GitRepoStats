...
class RepoManagement:
    ...
    def __init__(self, repo_path: str, log_box, period: PeriodFilter):
    ...
    def __iter_filtered_commits(self, **kwargs):
        """Itera i commit applicando eventuali filtri di data."""
    ...
    def __get_author_stats(self, author: Author) -> AuthorStats:
        ...
        for email in author.emails:
            for commit in self.__iter_filtered_commits(author=email):
    ...
    def __get_file_stats(self, file_name: str, authors: list[Author]) -> FileStats:
        ...
        for commit in self.__iter_filtered_commits(paths=file_name):
            changes += 1
    ...
    def __plot_all_stats(self, all_stats: list[AuthorStats], file_stats: list[FileStats], commits_stats: list[CommitStats], branches_stats: list[BranchStats]) -> None:
        ...
        data_to_plot = Data(self.repo_name, self.period, authors_html, files_html, languages_html, commits_html, comulative_commits_html, branches_html, comulative_branches_html, csv_files, csv_branches)
